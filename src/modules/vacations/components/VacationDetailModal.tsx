...
import { getVacationData, checkVacationRights, createVacationRequest, deleteVacationRequest } from '../actions';
import { format, addDays, differenceInDays } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { toast } from 'sonner';
import { Calendar, History, ShieldAlert, BadgeCheck, Clock, Info, Trash2 } from 'lucide-react';
...
export function VacationDetailModal({ isOpen, onClose, employeeId, employeeName }: VacationDetailModalProps) {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Gestão de Férias: ${employeeName}`} width="5xl">
            <div className="grid md:grid-cols-12 gap-6">
                        ...
                        {(() => {
                            ...
                            const uniquePeriods = periods.reduce((acc: any[], current: any) => {
                                ...
                                if (!exists) {
                                    ...
                                    // Merge requests if duplicated for some reason
                                    exists.requests = [...(exists.requests || []), ...(current.requests || [])];
                                    // Deduplicate merged requests by ID
                            ...
                            return uniquePeriods.map(period => {
                                ...
                                // Real-time Status Detection for Period
                                const now = new Date();
                ...
                <div className="md:col-span-7 space-y-6">
                    {selectedPeriod && (
                        <Card className="border-slate-100 dark:border-slate-800 shadow-sm rounded-2xl overflow-hidden bg-white dark:bg-slate-950">
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1.5">
                                            ...
                                            <input
                                                type="date"
                                                required
                                                className="w-full bg-slate-50 border border-slate-100 dark:bg-slate-900 dark:border-slate-800 p-2.5 rounded-xl text-sm font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-[#FF7800]/20 transition-all outline-none"
                                                value={startDate}
                                    ...
                                    {startDate && daysCount > 0 && selectedPeriod && (
                                        <div className="p-3 bg-orange-50 dark:bg-orange-950/20 rounded-xl border border-orange-100 dark:border-orange-900/50 space-y-2">
                                            ...
                                            <div className="flex justify-between items-center border-t border-orange-100 dark:border-orange-900/30 pt-2">
                                                <span className="text-[9px] font-black text-orange-700 dark:text-orange-400 uppercase">Saldo após agendamento</span>
                                                <span className="text-xs font-black text-orange-900 dark:text-orange-200">
                                                    {(() => {
                                                        const p = periods.find(p => p.id === selectedPeriod);
                                                        const used = p?.requests.reduce((acc: number, r: any) => acc + r.daysCount + r.soldDays, 0) || 0;
                                                        const remaining = 30 - used - (Number(daysCount) + Number(soldDays));
                    ...
                    <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 flex items-start gap-4">
