            ...
            <div className="lg:col-span-4 lg:sticky lg:top-8 self-start">
                <div className="bg-surface/60 backdrop-blur-xl border border-border rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="relative space-y-6">
                        <div>
                            <h3 className="text-sm font-black text-text-primary uppercase tracking-[0.2em]">Agendar <span className="text-brand-orange">Feriado</span></h3>
                            <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest mt-1">Configuração de Calendário Oficial</p>
                        </div>

                        <div className="space-y-4 pt-4">
                            <div className="space-y-2 group">
                                <label className="text-[9px] font-black text-text-muted uppercase tracking-widest ml-1">Data de Referência</label>
                                <Input
                                    type="date"
                            ...
                            <div className="space-y-2 group">
                                <label className="text-[9px] font-black text-text-muted uppercase tracking-widest ml-1">Título do Evento</label>
                                <Input
                ...
                <div className="bg-surface/40 backdrop-blur-xl border border-border rounded-[2.5rem] p-8 shadow-2xl relative">
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            <AnimatePresence mode="popLayout">
                                {holidays.map((h, i) => (
                                    <motion.div
                                        ...
                                        className="grid grid-cols-12 items-center px-8 py-5 bg-surface border border-border rounded-[1.5rem] hover:border-brand-orange/30 hover:scale-[1.01] hover:bg-text-primary/[0.02] transition-all duration-300 group"
                                    >
                                        <div className="col-span-4 flex items-center gap-4">
                                            <div className="w-10 h-10 rounded-xl bg-text-primary/5 flex flex-col items-center justify-center border border-border group-hover:border-brand-orange/30 transition-colors">
                                                <span className="text-[14px] font-black text-text-primary font-mono leading-none">
                                                    {new Date(h.date).getUTCDate()}
                                                </span>
                                                <span className="text-[7px] font-black text-text-muted uppercase">
                                                    {new Date(h.date).toLocaleDateString('pt-BR', { month: 'short', timeZone: 'UTC' })}
                                                </span>
                                            </div>
                                            <span className="text-[11px] font-bold text-text-muted/60 font-mono">
                                                {new Date(h.date).toLocaleDateString('pt-BR', { year: 'numeric', timeZone: 'UTC' })}
                                            </span>
                                        </div>

                                        <div className="col-span-6 min-w-0">
                                            <h4 className="text-[13px] font-black text-text-primary uppercase tracking-tight group-hover:text-brand-orange transition-colors truncate">
                                                {h.name}
                                            </h4>
                                        </div>
                            ...
                            {!loading && holidays.length === 0 && (
                                <div className="flex flex-col items-center justify-center py-20 text-text-muted/40 bg-text-primary/2 rounded-[2.5rem] border border-border border-dashed">
