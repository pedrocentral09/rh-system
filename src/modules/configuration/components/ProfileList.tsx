...
export function ProfileList() {
    ...
    if (loading) return (
        <div className="space-y-6 mt-8">
            <div className="h-16 bg-text-primary/5 rounded-3xl animate-pulse" />
            <div className="h-64 bg-text-primary/5 rounded-3xl animate-pulse" />
        </div>
    );
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700 mt-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h2 className="text-2xl font-black text-text-primary uppercase tracking-tighter">Perfis & <span className="text-brand-orange">Autoridade</span></h2>
                    <p className="text-text-muted text-xs font-bold uppercase tracking-widest mt-1">Matriz de Permissões e Hierarquia Funcional</p>
                </div>
                <button
                    onClick={() => {
                    ...
                    <div className="space-y-3">
                        {roles.map((role, i) => (
                            <motion.div
                                ...
                                className="grid grid-cols-12 items-center px-8 py-6 bg-surface border border-border rounded-[1.5rem] hover:border-indigo-500/30 hover:scale-[1.01] hover:bg-text-primary/[0.02] transition-all duration-300 group relative overflow-hidden"
                            >
                                <div className="col-span-12 md:col-span-6 flex items-center gap-5">
                                    <div className="w-12 h-12 rounded-2xl bg-text-primary/5 border border-border flex items-center justify-center text-xl shadow-inner group-hover:border-indigo-500/30 transition-colors">
                                        🎭
                                    </div>
                                    <div className="min-w-0">
                                        <h4 className="text-[14px] font-black text-text-primary uppercase tracking-tight group-hover:text-indigo-400 transition-colors truncate">{role.name}</h4>
                                        <div className="flex items-center gap-2 mt-0.5">
                                            {role.isSystem ? (
                                                ...
                                                <span className="text-[8px] font-black text-text-muted/60 uppercase tracking-widest bg-text-primary/5 px-2 py-0.5 rounded border border-border">Modelo Customizado</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="hidden md:block md:col-span-3 text-center">
                                    <div className="inline-flex items-center gap-3 bg-text-primary/5 px-4 py-2 rounded-2xl border border-border">
                                        <span className="text-sm font-black text-text-primary">{role._count?.users || 0}</span>
                                        <span className="text-[8px] font-black text-text-muted uppercase tracking-widest">Contas Ativas</span>
                                    </div>
                                </div>
