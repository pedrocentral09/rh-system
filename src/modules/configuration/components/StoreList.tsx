    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h2 className="text-2xl font-black text-text-primary uppercase tracking-tighter">Gestão de <span className="text-brand-orange">Unidades</span></h2>
                    <p className="text-text-muted text-xs font-bold uppercase tracking-widest mt-1">Configuração de Lojas e Pontos Físicos</p>
                </div>
                <button
                    onClick={() => openModal()}
                            ...
                            >
                                <div className="col-span-12 md:col-span-5 flex items-center gap-5">
                                    ...
                                    <div className="min-w-0">
                                        <h4 className="text-[14px] font-black text-text-primary uppercase tracking-tight group-hover:text-brand-orange transition-colors truncate">{store.name}</h4>
                                ...
                                <div className="hidden md:block md:col-span-3">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            <p className="text-[11px] font-black text-text-primary/80 uppercase tracking-tighter truncate max-w-[150px]">
                                                {store.manager?.name || store.responsible || 'Sem Gerente'}
                                            </p>
                                            <p className="text-[9px] font-bold text-text-muted uppercase tracking-widest mt-0.5">{store.city || 'Sede'} - {store.state || 'UF'}</p>
                                        </div>
                                    </div>
                                </div>

                                <div className="hidden md:block md:col-span-2 text-center">
                                    <div className="inline-flex flex-col items-center bg-text-primary/5 px-4 py-2 rounded-2xl border border-border">
                                        <span className="text-sm font-black text-text-primary">{store._count?.contracts || 0}</span>
                                        <span className="text-[8px] font-black text-text-muted uppercase tracking-widest">Ativos</span>
                                    </div>
                                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8 py-6 max-h-[75vh] overflow-y-auto px-4 custom-scrollbar">
                    <div className="space-y-6">
                        <div className="text-center">
                            <h4 className="inline-block text-[10px] font-black text-text-muted uppercase tracking-[0.3em] border-b border-border pb-2 mb-8">Identificação Estratégica</h4>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2 group">
                                <label className="text-[9px] font-black text-text-muted uppercase tracking-widest ml-1 group-focus-within:text-brand-orange transition-colors">CNPJ</label>
                                <Input
                                    className="bg-text-primary/5 border-border rounded-2xl h-12 text-sm font-bold text-text-primary focus:border-brand-orange/30 transition-all"
                                    value={formData.cnpj}
                            ...
                            <div className="space-y-2 group">
                                ...
                                <select
                                    className="w-full h-12 bg-text-primary/5 border border-border rounded-2xl px-4 text-sm font-bold text-text-primary outline-none focus:border-brand-orange/30 transition-all appearance-none cursor-pointer"
                                    value={formData.managerId}
                                    onChange={e => handleChange('managerId', e.target.value)}
                                >
                                    <option value="" className="bg-surface">SELECIONE UM GERENTE...</option>
                                    {employees.map(emp => (
                        ...
                        <div className="text-center pt-6">
                            <h4 className="inline-block text-[10px] font-black text-text-muted uppercase tracking-[0.3em] border-b border-border pb-2 mb-8">Logística & localização</h4>
                        </div>
