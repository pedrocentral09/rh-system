...
import { getCostByStore } from '../actions/analytics';
import { Card, CardContent, CardHeader, CardTitle } from '@/shared/components/ui/card';
import { Loader2, Store } from 'lucide-react';

export function StoreCostBreakdown({ periodId }: { periodId: string }) {
    ...
    useEffect(() => {
        async function load() {
            setLoading(true);
            const res = await getCostByStore(periodId);
    ...
    return (
        <Card className="border-none shadow-md overflow-hidden bg-white dark:bg-slate-950">
            <CardHeader className="bg-amber-50 dark:bg-amber-950/20 border-b border-amber-100 dark:border-amber-900/50">
                <CardTitle className="text-amber-700 dark:text-amber-400 flex items-center text-lg">
                    <Store className="mr-2 h-5 w-5" /> Custos por Loja / Unidade
                </CardTitle>
            </CardHeader>
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-900">
                        {data.map(item => (
                            <tr key={item.name} className="hover:bg-slate-50 dark:hover:bg-slate-900/30">
                                ...
                                <td className="px-4 py-3 text-right font-black text-amber-600">{formatCurrency(item.value)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
