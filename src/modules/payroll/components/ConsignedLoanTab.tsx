...
export function ConsignedLoanTab({ periodId, isClosed }: LoanTabProps) {
    ...
    async function handleAdd() {
        if (!selectedEmployeeId || !totalAmount || !installmentsCount || !startMonth) {
            ...
            {isAdding && (
                <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border-2 border-indigo-100 dark:border-indigo-900/30 shadow-lg animate-in slide-in-from-top-4">
                    <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                        <div className="md:col-span-1">
                            ...
                            <Popover open={openPopover} onOpenChange={setOpenPopover}>
                                <PopoverTrigger asChild>
                                    <Button variant="outline" role="combobox" className="w-full justify-between font-normal px-2">
                                        <span className="truncate">{selectedEmployeeId ? employees.find(e => e.id === selectedEmployeeId)?.name?.split(' ')[0] : "Selecionar..."}</span>
                        ...
                        <div className="md:col-span-1">
                            <label className="block text-xs font-bold text-slate-500 uppercase mb-1" title="Mês e Ano da Primeira Parcela">Início</label>
                            <input
                                type="month"
                                className="w-full p-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm bg-slate-50 dark:bg-slate-900"
                                value={startMonth}
                        ...
                        <div className="md:col-span-1 flex space-x-2">
                            <Button onClick={handleAdd} className="flex-1 bg-emerald-600 hover:bg-emerald-700 px-2 text-xs">Salvar</Button>
                            <Button variant="ghost" onClick={() => setIsAdding(false)} className="px-2">X</Button>
                        </div>
                    </div>
                </div>
            )}
